...
#'
#' @param leipzig_wlist_path the file paths of the Leipzig wordlist files.
...
#' tb <- freqlist_leipzig_read(leipzig_wordlist_path,
#'                             file_pattern = "(web_2012|wikipedia_2016)",
#'                             all = FALSE,
#'                             lower_case = TRUE)
#'
#'  # subset only on word that consists of letters and hypen
#'  # to retrieve/maintain reduplication (e.g., "anak-anak" 'children')
#'  dplyr::filter(tb, stringr::str_detect(word, "^([-a-z]+)$"))
#' }


freqlist_leipzig_read <- function(leipzig_wlist_path = leipzig_wordlist_path, file_pattern = NULL, all = FALSE, lower_case = TRUE) {
  ...
  if (all == TRUE) {
    wlist_corpus_id <- basename(leipzig_wlist_path)
    wlist_df <- tibble::tibble()
    for (i in seq_along(leipzig_wlist_path)) {
      temp_wlist <- readr::read_table(file = leipzig_wlist_path[i], col_names = FALSE)
    ...
    if (is.null(file_pattern)) {
      ...
      wlist_files <- stringr::str_subset(leipzig_wlist_path, file_pattern)
      wlist_corpus_id <- stringr::str_subset(basename(leipzig_wlist_path), file_pattern)
      wlist_df <- tibble::tibble()
