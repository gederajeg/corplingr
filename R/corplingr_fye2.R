#' Iterative \emph{Fisher-Yates Exact} test
#'
...
#'     \code{fye2} provides a logical argument (i.e., \code{two_sided}) whose value is passed to the \code{alternative} argument of the embedded \code{\link[stats]{fisher.test}}.
...
#' @param two_sided whether to perform one-sided test (\code{FALSE} -- Default) or two-sided (\code{TRUE}).
#' @param res choices of outputs: (i) the \code{p.value} ("p") or (ii) \code{odds.ratio} ("or") of the \code{\link[stats]{fisher.test}}.
#' @param float the floating digits of \code{res} (\code{3} -- the default).
...
fye2 <- function (a, b, c, d, a.exp, two_sided = FALSE, res = c("p", "or"), float = 3) {
  ...
  for (i in seq_along(a)) {
    ...
    if (two_sided == FALSE) { # perform one-tailed FYE test
      ...
      if ((a[i] >= a.exp[i]) == TRUE) { # check if the obs.freq is more than exp.freq; if yes, take 'greater' alternative in the FYE
        output <- stats::fisher.test(cross_table, alternative = 'greater') # one-tailed FYE for more-than-exp obs.freq
      } else { # check if the obs.freq is less than exp.freq; if yes, take 'less' alternative in the FYE
        output <- stats::fisher.test(cross_table, alternative = 'less') # one-tailed FYE for less-than-exp obs.freq
      }

    } else { # perform two-tailed FYE test
      output <- stats::fisher.test(cross_table, alternative = "two.sided")
    }
    if (res %in% c("pval", "p.value", "pvalue", "p")) {
      results[i] <- as.numeric(format(output$p.value, digits = float + 1))
    } else if (res %in% c("or", "odds.ratio", "OR")) {
      results[i] <- round(output$estimate, digits = float)
    }
  }
  return(results)
}
